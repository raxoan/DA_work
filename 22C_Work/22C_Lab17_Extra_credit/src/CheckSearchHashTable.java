...
//				 System.out.println("The returned value is: " + found);
				...
				if (referenceResult == null) {
					if (found != null) {
						failed = true;
						System.out.println("    fails - item: " + k + " should not have been found" + "at position = " + found);
					}
				} else {
					if (!referenceResult.equals(found)) {
						failed = true;
						System.out.println(
								"    fails - item: " + k + " should have been found; got " + found + " instead");
...
//				 System.out.println("The returned value is: " + found);
				...
				if (referenceResult == null) {
					if (found != null) {
						failed = true;
						System.out.println("    fails - item: " + k + " should not have been found");
					}
				} else {
					if (!referenceResult.equals(found)) {
						failed = true;
...
//			System.out.println("The data after addition inserts is: " + table.toString());
			System.out.println("The reference after addition inserts is: " + reference);
...
//				 System.out.println("The returned value is: " + found);
	...
	/**
	 * Generate an array of random integer values. The values will be between 0 and
	 * size.
	 *
	 ...
	 * @param keys        An array of objects that will be the keys for insertion
	 *                    into the dictionary. The value will just be the key
	 *                    itself.
	 */
	private static void insertFirstHalfIntoDictionary(DictionaryInterface<Integer, Integer> aDictionary,
			Integer[] keys) {
	...
	/**
	 * Insert the first half of the data values from an array of objects into a tree
	 * map.
	 * 
	 * @param referenceMap A tree map that the values will be inserted into.
	 * @param keys         An array of integers that will be the keys for insertion
	 *                     into the map. The value will just be the key itself.
	 */
	 ...
	 *                    itself.
	 */
	private static void insertSecondHalfIntoDictionary(DictionaryInterface<Integer, Integer> aDictionary,
			Integer[] keys) {
		System.out.println("Running 'insertSecondHalfIntoDictionary'");
		for (int i = keys.length / 2 + 1; i < keys.length; i++) {
//			System.out.println("Adding : " + keys[i]);
			aDictionary.add(keys[i], keys[i]);
//			System.out.println(keys[i] + " was added");
		}
	}
	 ...
	 * @param keys        Ab array of integers that will be the keys for deletion
	 *                    from the dictionary.
	 */
	 ...
	 * @param keys         An array of objects that will be the keys for deletion
	 *                     the map.
	 */
	...
	/**
	 * A displayable representation of an array of Objects where toString is applied
