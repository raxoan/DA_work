 ...
 * 
 * This code is from Chapter 14 of Data Structures and Abstractions with Java
 * 4/e
 * 
...
/**
 * 
 * @author Aaron Wong CIS 22C - Lab 13 - ListImplementation LList.java
 * @param <T>
 */
...
class LList<T> implements ListInterface<T> {
	...
	public void reverse() {
		if (numberOfEntries < 2) {
			return; // list of length 0 or 1 cannot be reversed
		} else if (numberOfEntries == 2) { // reversing a list of length 2 is simple and different than lists of length
											// 3+
			firstNode.getNextNode().setNextNode(firstNode);
	...
	public void cycle() {
		if (numberOfEntries < 2) {
			...
			Node start = firstNode; // save the first node to attach at the end
			Node current = firstNode;
			while (current.getNextNode() != null) { // iterate through the list
				current = current.getNextNode();
			}
			firstNode = firstNode.getNextNode(); // set the firstNode variable to the second node
			current.setNextNode(start); // set the lastNode.next to the starting node
			start.setNextNode(null); // set the new lastNode.next to null.
		}
