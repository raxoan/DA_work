...
public final class AdventureGame implements InterfaceList {
	...
	public static void main(String[] args) {
		...
		try {
			...
			User u = new User(0, 20, username, 20, 20, 15, 7, 8);
	...
	public void readFile(Scanner input) throws IOException {
		...
		while (input.hasNextLine()) {
			...
			Enemy e = new Enemy(new StringBuilder(name), hp, armor, speed, damage);
			enemies.add(e);
			...
			while (!turn.equalsIgnoreCase("R")) {
				...
				if (turn.equalsIgnoreCase("A")) {
					...
					if (userHit > e.getArmor()) {
						...
						dmgR = u.rollDmg(u.getDamage()) + 2; // User rolls a d8 for damage, +2 bonus
						...
						if (e.getHp() == 0 && u.getHp() != 0) {
							...
							Enemy.updateScore(u.getHp()); // If enemy loses, user earns a score based on current health point
			...
			while (!turn.equalsIgnoreCase("R")) {
				...
				if (turn.equalsIgnoreCase("A")) {
					...
					if (userHit > e.getArmor()) {
						...
						dmg = u.rollDmg(u.getDamage()) + 2; // User rolls a d8 for damage, adding a +2 bonus
