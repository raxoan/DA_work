...
public class CountingGame {
	...
	public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt) {
		...
		int pNum = 0; // counter for player list
		int maxP = players.getLength(); // max number of players
		for (int i = 1; i <= rhyme.getLength(); i++) {
			pNum++;
			if (pNum > maxP) { // if there are more words than players, the loop needs to restart at the beginning of the player list
				pNum = 1;
			}
			System.out.println("Word: " + rhyme.getEntry(i) + " - Player: " + players.getEntry(pNum));
			
			
		}
		System.out.println("Removing Player " + (pNum));
		players.remove(pNum);
		return pNum;
