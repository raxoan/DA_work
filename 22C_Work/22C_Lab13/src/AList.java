 ...
 * 
 * This code is from Chapter 13 of Data Structures and Abstractions with Java
 * 4/e
 * 
 * @author Frank M. Carrano
 * 
 *         The toString method is overwritten to give a nice display of the
 *         items in the list in this format { <1> <2> <3> <4> } Modification by
 *         Charles Hoot
 * 
 ...
 * 
 * @author Aaron Wong
 * CIS 22C - Lab 13 - ListImplementation
 * AList.java
 * @param <T>
 */
...
class AList<T> implements ListInterface<T> {
	...
	public void reverse() {
		if (numberOfEntries < 2) {
			return; // a list with 0 or 1 item does not need to be reversed
		} else {
			for (int i = 0; i < (numberOfEntries/2); i++) { // iterate through half of the list
				T temp = list[i + 1]; // save the item at the beginning of the list in a temp field
				list[i + 1] = list[numberOfEntries - i]; // replace the item at the beginning of the list with an item at the end of the list
				list[numberOfEntries - i] = temp; // place the temp value at the end of the list
	...
	public void cycle() {
		// COMPLETE THIS METHOD
		if (numberOfEntries < 2) {
			return; // a list with 0 or 1 item cannot be cycled
		} else {
			T temp = list[1]; // save the first item of the list
			removeGap(1);  // move all entries down by 1 
			list[numberOfEntries] = temp; // place the saved temp variable at the end of the list
		}
	}
